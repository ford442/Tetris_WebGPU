...
- "Audited hardDrop visual pipeline. Removed redundant \`this.gameStateCache.effectFlag = true;\` in \`game.ts\` because \`performHardDrop\` already triggers it accurately. Confirmed that \`viewRenderLoop.ts\` is correctly unconditionally advancing \`lastEffectCounter\` when \`effectFlag\` is set without missing pulse boosts."
