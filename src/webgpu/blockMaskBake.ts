/**
 * Metal/glass mask bake — the pure, canvas-free core of the tile extractor.
 *
 * Split out of blockTextureExtract.ts so the same code runs in three places:
 *   * the browser extractor (canvas in, canvas out),
 *   * the CPU contract tests, which decode public/block.png and assert the gold
 *     frame and crystal well still satisfy block-material.json,
 *   * the Mask Lab, when tuning the heuristic.
 *
 * Semantics are unchanged from when this lived inline: the alpha channel of the
 * extracted tile carries the metal mask (255 = gold frame, 0 = crystal).
 */

import type { BlockTextureConfig } from './blockTexture.js';

/**
 * Bake the metal/glass segmentation into the alpha channel of an extracted tile,
 * in place. Pure: no canvas, no GPU, so the authored-material contract can be
 * measured on CPU in `npm test` (tests/block-material-contract.test.ts) instead of
 * only by looking at a screenshot.
 *
 * `maskData` is an already-cropped companion mask (same dimensions as the tile).
 * When absent the heuristic path runs: warmth/colour signal, Otsu threshold, then a
 * flood fill that keeps only the metal component connected to the border ring.
 */
export function bakeMetalMaskAlpha(
  data: Uint8ClampedArray | Uint8Array,
  width: number,
  height: number,
  config: BlockTextureConfig,
  maskData: Uint8ClampedArray | Uint8Array | null = null,
): void {
  ...
  if (maskData) {
    ...
    applyFeatheredAlpha(data, width, height, dilated, featherPx);
    return;
  }
  ...
  if (config.materialDetectionMode === 'none' || config.materialDetectionMode === 'alpha') {
    ...
    const dilatePx = Math.max(0, Math.round(config.maskDilatePx ?? 1));
    const dilated = dilateBinaryMask(keep, width, height, dilatePx);
    applyFeatheredAlpha(data, width, height, dilated, Math.max(0, Math.round(config.maskFeatherPx ?? 1)));
    return;
  }
...
function applyFeatheredAlpha(
  ...
  // Write blurred mask back into the alpha channel.
  for (let i = 0; i < n; i++) {
